...
my $startTime = strftime "%Y-%m-%dT%H:%M:%S.000Z", gmtime ( time - 300);
my $endTime = strftime "%Y-%m-%dT%H:%M:%S.000Z", gmtime;

my $url = "https://$Hostname/rest/appliance/monitoring/query?item.interval=MINUTES5&item.function=AVG&item.start_time=$startTime&item.end_time=$endTime&item.names.1=cpu.util";
